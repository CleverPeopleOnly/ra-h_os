...
export interface EdgeData {
  ...
  // Belief-engine evidence fields (MR-A). When evidence_relation is set the
  ...
  evidence_relation?: 'supports' | 'contradicts'; // how the from-node bears on the to-node
  evidence_strength?: number;                     // evidence weight in [0, 1]
  evidence_independence_key?: string | null;      // same key = non-independent evidence (POLICY V1 collapses)
}
