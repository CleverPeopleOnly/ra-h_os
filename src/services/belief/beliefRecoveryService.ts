 ...
 * trusted. A support-bearing edge runs in RA-H's canon direction,
 * Derivative→Source (spec §8): the node whose grade may be stale is always
 * the edge's FROM-end — the derived node — and the source credence lives on
 * the edge's TO-end. It recovers three shapes:
 *
 ...
 *  3. MODEL-MIGRATION shape (new in v2): a node with an evidence basis whose
...
// regrade would leave it untouched anyway, and its unstamped evidence basis
// matches the pending-work condition on every run, so without the
...
export async function runBeliefRecoverySweep(): Promise<BeliefRecoveryResult> {
  ...
  // Shape 1 — never-stamped evidence: derived nodes (from-ends) carrying at
  // least one evidence edge with a NULL contribution stamp (support set,
  // stamp NULL — i.e. offline writes). A NULL support means the edge is not
  // evidence, so it is never pending work.
  ...
  // for the stamp to agree with, so it is stale by definition). The source
  // credence is read from the edge's to-end — the node the derived from-end
  // derives from.
  const staleStampedEvidenceNodeRows = sqlite
    .prepare(
      `SELECT DISTINCT e.from_node_id AS node_id
       FROM edges e
       JOIN nodes derived ON derived.id = e.from_node_id
       JOIN nodes source ON source.id = e.to_node_id
       WHERE e.belief_evidence_support IS NOT NULL
         AND e.belief_evidence_contribution IS NOT NULL
         AND derived.belief_credence_is_fixed = 0
         AND (
  ...
  // Shape 3 — model migration: a node with an evidence basis graded under v1
  // (credence stored, masses never written). Restricted to nodes that HAVE
  // an evidence basis — outgoing support-bearing edges — because only such
  // nodes were ever graded by the engine: a credence with no evidence behind
  // it has nothing to regrade from, and regrading it would clear it rather
  // than migrate it.
  const massLessGradedNodeRows = sqlite
    .prepare(
      `SELECT DISTINCT n.id AS node_id
       FROM nodes n
       JOIN edges e ON e.from_node_id = n.id AND e.belief_evidence_support IS NOT NULL
