/**
 * Tests for the NEW belief tools on the REMOTE MCP door
 ...
 * edge-side belief surface — no way to bootstrap a credence or read a node's
 * movement history. The door is an HTTP proxy: each tool validates at its
 * schema, forwards to the app's belief endpoints
 * (POST /api/belief/fixed-credence, GET /api/belief/movements — pinned
 * behaviourally in tests/unit/api/), and passes the app's answer through.
 * The app is never reimplemented door-side.
 *
 * deleted in the display-belief-door-writable slice: the
 * rah_recompute_node_belief describe, its stub branch and its entry in
 * newBeliefToolNames — the recompute surface is dead; the door's writable
 * belief surface is rah_write_display_belief (pinned in
 * remote-mcp-route-display-belief-tool.test.ts).
 *
...
// The new belief tool names, exactly as both doors must advertise them.
...
describe('remote MCP door advertises the new belief tools', () => {
  ...
  it('lists the new belief tools with input and output schemas', async () => {
    await remoteMcpDoorHarness.withRemoteMcpClient(async (client) => {
  ...
  it('names the new belief tools in the GET metadata tool list', async () => {
