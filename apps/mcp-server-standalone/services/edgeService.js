 ...
 *        from_node_id, 'into' means the node is the to_node_id, 'both' is
...
function createEdge(edgeData) {
  // An edge is a plain knowledge-graph relationship, so the write reads only
  // the relationship fields — a stray key a stale caller still sends
  // (belief_evidence_support was one) is simply not part of the write.
  const {
    from_node_id,
    to_node_id,
    explanation,
